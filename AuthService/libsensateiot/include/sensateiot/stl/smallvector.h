...
// License. See LICENSE.TXT for internals.
//
//===----------------------------------------------------------------------===//
//
...
/// to avoid unnecessarily requiring T to be complete.
	...
	private:
		template<typename, unsigned> friend
		struct SmallVectorStorage;

		// Allocate raw space for N elements of type T.  If T has a ctor or dtor, we
		// don't want it to be automatically run, so we need to represent the space as
		// something else.  Use an array of char of sufficient alignment.
		////////////typedef sensateiot::stl::AlignedCharArrayUnion<T> U;
...
// Define this out-of-line to dissuade the C++ compiler from inlining it.
	...
	void SmallVectorTemplateBase<T, isPodLike>::grow(size_t MinSize)
	{
		...
		size_t NewCapacity = size_t(sensateiot::stl::internal::NextPowerOf2(CurCapacity + 2));
...
/*
  ...
  explicit SmallVector(const sensateiot::stl::iterator_range<RangeTy> &R)
      : SmallVectorImpl<T>(N) {
...
namespace std
{
	...
	template<typename T>
	inline void
	swap(sensateiot::stl::SmallVectorImpl<T> &LHS, sensateiot::stl::SmallVectorImpl<T> &RHS)
	{
		LHS.swap(RHS);
	}
	...
	template<typename T, unsigned N>
	inline void
	swap(sensateiot::stl::SmallVector<T, N> &LHS, sensateiot::stl::SmallVector<T, N> &RHS)
	{
		LHS.swap(RHS);
	}
}
