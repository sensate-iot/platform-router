...
namespace sensateiot::util
{
	...
	class contention_free_shared_mutex {
		...
		enum index_op_t {
			unregister_thread_op, get_index_op, register_thread_op
		};
		...
		struct unregister_t {
			...
			unregister_t(int index, std::shared_ptr<array_slock_t> const &ptr) : thread_index(index),
			                                                                     array_slock_ptr(ptr)
			{
			}

			unregister_t(unregister_t &&src) : thread_index(src.thread_index),
			                                   array_slock_ptr(std::move(src.array_slock_ptr))
			{
			}
	...
	public:
		contention_free_shared_mutex() :
				shared_locks_array_ptr(std::make_shared<array_slock_t>()), shared_locks_array(*shared_locks_array_ptr),
				want_x_lock(false), recursive_xlock_count(0),
		...
		void lock_shared()
		{
			...
			if(register_index >= 0) {
				...
				if(owner_thread_id.load(std::memory_order_acquire) != get_fast_this_thread_id()) {
					size_t i = 0;
					for(bool flag = false; !want_x_lock.compare_exchange_weak(flag, true,
					                                                          std::memory_order_seq_cst); flag = false) {
						if(++i % 100000 == 0)
							std::this_thread::yield();
					}
	...
	struct shared_lock_guard {
		...
		shared_lock_guard(mutex_t &mtx) : ref_mtx(mtx)
		{
			ref_mtx.lock_shared();
		}
	...
	using shared_mutex = contention_free_shared_mutex<>;
	// ---------------------------------------------------------------
}
