...
namespace SensateIoT.Platform.Ingress.MqttService.Mqtt
{
	...
	{
		private readonly ILogger<MqttMeasurementHandler> m_logger;
		private readonly IHttpClientFactory m_factory;
		private readonly IServiceProvider m_provider;
		private readonly GatewaySettings m_settings;

		public MqttMeasurementHandler(IHttpClientFactory factory,
									  IServiceProvider provider,
									  IOptions<GatewaySettings> settings,
									  ILogger<MqttMeasurementHandler> logger)
		{
		...
		{
			try {
				this.m_logger.LogInformation("Received measurement via MQTT.");
				...
				if(rawId == null) {
					this.m_logger.LogWarning("Measurement has no sensor ID.");
					return;
				}
				...
				client.BaseAddress = new Uri(this.m_settings.Measurements);
