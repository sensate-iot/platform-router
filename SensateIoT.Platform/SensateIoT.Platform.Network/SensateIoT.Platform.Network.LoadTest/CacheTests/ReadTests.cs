...
namespace SensateIoT.Platform.Network.LoadTest.CacheTests
{
	public class ReadTests
	{
		...
		{
			var cache = new MemoryCache<string, string>(MemoryCache<string, string>.DefaultCapacity, TimeSpan.FromMilliseconds(DefaultTimeout));
			var kvp = new List<Common.Caching.Abstract.KeyValuePair<string, string>>(size);

			for(var idx = 0; idx < size; idx++) {
				kvp.Add(new Common.Caching.Abstract.KeyValuePair<string, string> {
					Key = $"key::{idx}",
					Value = $"value::{idx}"
				});
			}
